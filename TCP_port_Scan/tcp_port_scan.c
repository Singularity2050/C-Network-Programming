        ...
        int sock, struct sockaddr *addr, unsigned long len, int sec)
{
...
#else
    if(fcntl(sock, F_SETFL, O_NONBLOCK) < 0) {// asynchronous
		perror("fcntl ");
		return -1;
	}
#endif

    FD_ZERO(&fread);//initialize
    FD_ZERO(&fwrite);
    ...
    if(connect(sock, addr, len) < 0) { //3 Way Handshake
#ifndef __linux__
    ...
    // The nfds argument specifies the range of file descriptors to be tested. (generally socket descriptor + 1)
